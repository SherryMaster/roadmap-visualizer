...
const AssemblerResults = ({ 
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `${mergedRoadmap.title.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_roadmap.json`;
  ...
  const handleSaveAndView = async () => {
    ...
    try {
      ...
      const roadmapId = RoadmapPersistence.saveRoadmap(mergedRoadmap, {
        source: 'assembler',
    ...
    <div className="space-y-6">
      ...
      <div className="text-center bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-6">
        ...
        <p className="text-green-700 dark:text-green-300">
          Your roadmap has been merged and validated. You can now download it or view it in the visualizer.
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm border border-gray-200 dark:border-gray-700">
        ...
        <div className="space-y-3">
          <div>
            ...
            <span className="ml-2 text-gray-900 dark:text-gray-100">{mergedRoadmap.title}</span>
          </div>
          <div>
            <span className="font-medium text-gray-700 dark:text-gray-300">Description:</span>
            <span className="ml-2 text-gray-900 dark:text-gray-100">{mergedRoadmap.description || 'No description'}</span>
          </div>
          <div>
            <span className="font-medium text-gray-700 dark:text-gray-300">Project Level:</span>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm border border-gray-200 dark:border-gray-700">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">{mergeStats.totalPhases}</div>
            <div className="text-sm text-gray-600 dark:text-gray-400">Phases</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-green-600 dark:text-green-400">{mergeStats.totalTasks}</div>
            <div className="text-sm text-gray-600 dark:text-gray-400">Tasks</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-purple-600 dark:text-purple-400">{mergeStats.taskFiles}</div>
            <div className="text-sm text-gray-600 dark:text-gray-400">Task Files</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-orange-600 dark:text-orange-400">{mergeStats.mergedTasks}</div>
            <div className="text-sm text-gray-600 dark:text-gray-400">Merged Tasks</div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm border border-gray-200 dark:border-gray-700">
        ...
        <div className="space-y-3">
          <div className="flex items-center space-x-2">
            ...
            <span className={validationResults.skeleton?.isValid ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}>
          ...
          {validationResults.tasks?.map((taskResult, index) => (
            <div key={index} className="flex items-center space-x-2">
              ...
              <span className={taskResult.isValid ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}>
          ...
          <div className="flex items-center space-x-2">
            ...
            <span className={validationResults.overall?.isValid ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}>
              {validationResults.overall?.isValid ? 'All validations passed' : 'Some validations failed'}
            </span>
          </div>
        </div>
      </div>
